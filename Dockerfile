...
WORKDIR /go/src/github.com/cakoolen/markdown-index
RUN go get -d -v golang.org/x/net/html
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgop .

FROM alpine:latest
LABEL org.opencontainers.image.source https://github.com/cakoolen/markdown-index
...
WORKDIR /app
COPY --from=builder /go/src/github.com/cakoolen/markdown-index/markdown-index .
